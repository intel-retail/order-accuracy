...
Why this is deterministic:
  ...
  ``src/mcp_service.py``'s ``emit_order_result()`` -> ``svc.emit()``).
...
uses), otherwise falls back to ``<repo>/dine-in/results/order_accuracy_events.db``
(the host-side path the ``results/`` bind mount maps to ``/app/results``
inside the container — see docker-compose.yaml).
"""
...
_DEFAULT_LOG_PATH = str(APP_DIR / "results" / "order_accuracy_events.db")
