...
# Upload root for user-submitted video files
...
def create_app() -> FastAPI:
    ...
    def recall_order(order_id: str):
        ...
        order_frame_dir = _find_order_frame_dir(station_id, safe_order_id)
...
def _find_order_frame_dir(station_id: str, order_id: str) -> Optional[Path]:
    ...
    candidate = (root_dir / safe_station_id / safe_order_id).resolve()
    if not _is_within_root(candidate, root_dir):
        logger.warning(
            f"[API] Rejected unsafe frame path candidate for station={station_id}, order={order_id}"
        )
        return None
    ...
    if FRAME_SELECTOR_DIR.exists():
        for station_dir in FRAME_SELECTOR_DIR.iterdir():
            if station_dir.is_dir():
                alt = (station_dir / safe_order_id).resolve()
                if not _is_within_root(alt, root_dir):
                    continue

                if alt.exists():
                    logger.debug(
                        f"[API] Found frames for order {safe_order_id} in {alt} "
