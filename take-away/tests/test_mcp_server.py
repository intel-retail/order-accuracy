"""Tests for the MCP server entrypoint (Issue #102 — Take-away).
...
def _reload_mcp_server(tmp_path):
    """Import fresh core.mcp_service + mcp_server modules against an
    isolated log file, mirroring test_mcp_service.py's _reload_mcp_service.
    """
    env = {
...
def test_describe_and_service_internals_unaffected(mcp_server_module):
    ...
    from core.mcp_service import svc
